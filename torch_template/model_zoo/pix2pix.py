...
class GlobalGenerator(nn.Module):
    def __init__(self, input_channel=3, output_channel=3, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
                 padding_type='reflect'):
        assert (n_blocks >= 0)
        super(GlobalGenerator, self).__init__()
        activation = nn.ReLU(True)

        model = [nn.ReflectionPad2d(3), nn.Conv2d(input_channel, ngf, kernel_size=7, padding=0), norm_layer(ngf), activation]
        ### downsample
        ...
        model += [nn.ReflectionPad2d(3), nn.Conv2d(ngf, output_channel, kernel_size=7, padding=0), nn.Tanh()]
